...
public class BarCharts {
  ...
  public void drawGraph(int min, int max, int graphType, int maxBarHeight1, FlightSchedule sampleFS)
  {
    ...
    for ( int i = min; i <= max; i++)
    {
      ...
      int barWidth = 30;
      maxBarHeight = maxBarHeight1; // larger this number , the taller the bars get 150,000


      int barHeight = (int) map(flightCount, 0, 1800000, 0, maxBarHeight);// smaller 18000 -> smaller bars
  ...
  public void axisTitles(int min, int max, String departureAirport, String arrivalAirport, int graphType, int maxX)
  {
    ...
    {
      String title = "Scheduled Flights Per Hour  ";
      text(title, 400, 50);
