class DoubleHandleSlider {
  ...
  public void controlEvent(ControlEvent event) {
    if (event.isFrom("sliderRange")) {
      ...
      lowDistValue = (int) min(values[0], values[1]);
      highDistValue = (int) max(values[0], values[1]);
      println("Slider handle moved. Low Distance value: " + lowDistValue + ", High Distance value: " + highDistValue);
    }
  }
}
